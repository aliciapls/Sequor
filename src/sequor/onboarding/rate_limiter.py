    ...
    def is_allowed(self, key: str) -> bool:
        ...
        if window is None:
            if len(self._windows) >= _MAX_TRACKED_KEYS:
                # Fail CLOSED, NOT open. Returning True here let an attacker who
                # first fills the map with _MAX_TRACKED_KEYS distinct keys (many
                # source IPs / spoofed X-Forwarded-For) bypass every throttle for
                # all subsequent new keys. Instead evict the oldest-INSERTED key
                # (FIFO via dict insertion order) and keep enforcing this one.
