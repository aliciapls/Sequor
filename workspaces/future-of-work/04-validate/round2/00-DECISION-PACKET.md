...
- **`test_contains_form` (last F-C4-05 unit failure)** — order-dependent isolation: `/` now serves `signup.html` which is a marketing landing (no inline form); the form fields live in `register.html`. The test asserts `/` contains `<form name="org_name"...>`. This is a product/UX question — should `/` serve the signup form, or is the marketing landing intended (repivot) with the form on a separate route/frontend? Not silently rewritten. → decide `/` content intent.
